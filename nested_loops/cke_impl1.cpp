...
struct NestedLoopKernel {
  ...
  KOKKOS_INLINE_FUNCTION void operator() (const Int iEdge, const Int k) const {
...
void run1 (const Data& d) {
  const auto f = KOKKOS_LAMBDA(const int idx) {
    int iEdge, k;
    d.idx_to_iEdge_kPack(idx, iEdge, k);
    const auto coef2 = d.normalThicknessFlux(iEdge,k)*d.advMaskHighOrder(iEdge,k);
    Data::Pr csgn; {
      const auto ntf = d.normalThicknessFlux(iEdge,k);
      vector_simd for (int s = 0; s < Data::packn; ++s)
  ...
  Kokkos::parallel_for(d.get_rpolicy_iEdge_kPack(), f);
  Kokkos::fence();
}
